...
import com.nautigsam.mineleapmod.helpers.McObfuscationHelper.JSyms;
...
public class JoypadControlList extends GuiScrollingList
{
	...
	private void drawControlButtons(int id, int x, int y, String bindingKey, boolean slotSelected)
	{
		...
		if (controlButtonStr.equals("NONE"))
		{
			if (binding != null)
			{
				if (binding.keyCodes != null && binding.keyCodes.length == 1 && binding.bindingOptions != null
						&& binding.bindingOptions.contains(ControllerBinding.BindingOptions.GAME_BINDING))
				{
		...
		if (enable)
		{
			...
			if (binding.inputEvent.getEventType() != EventType.AXIS
					&& !binding.bindingOptions.contains(ControllerBinding.BindingOptions.MENU_BINDING))
			{
				char toggle = McObfuscationHelper.symGet(McObfuscationHelper.JSyms.eCircle);
				if (binding.bindingOptions.contains(ControllerBinding.BindingOptions.IS_TOGGLE))
	...
	{
		ControllerBinding b = ControllerSettings.findControllerBindingWithKey(keyCode, ControllerBinding.BindingOptions.GAME_BINDING);
				...
				else if (lastXClick <= x + controlButtonWidth + smallButtonWidth * 2)
				{
							...
							&& !binding.bindingOptions.contains(ControllerBinding.BindingOptions.MENU_BINDING))
					{
						ControllerSettings.setToggle(parent.getCurrentControllerId(), binding.inputString,
								!binding.bindingOptions.contains(ControllerBinding.BindingOptions.IS_TOGGLE));
					}
				}
			}
		}
		...
		if (b == null)
		{
			...
			if (kb != null)
			{
				b = new ControllerBinding(bindingKey, bindingKey, new ButtonInputEvent(parent.getCurrentControllerId(),
						...
						ControllerBinding.BindingOptions.GAME_BINDING, ControllerBinding.BindingOptions.REPEAT_IF_HELD, ControllerBinding.BindingOptions.RENDER_TICK,
